...
const SKINS = [
  ...
  },
  {
    ...
    verts: [[24,0],[2,-6],[-10,0],[2,6]],
  },
  {
    name:  'FURTIVA',
    stroke: '#0f5',
    flame:  'rgba(0, 255, 120, 0.85)',
    verts: [[16,0],[-14,-12],[2,0],[-14,12]],
  },
  {
    name:  'AGUIJÓN',
    stroke: '#fa0',
    flame:  'rgba(255, 170, 0, 0.85)',
    verts: [[28,0],[0,-5],[-12,0],[0,5]],
  },
  {
    name:  'FÉNIX',
    stroke: '#f4f',
...
class PowerUp {
  ...
  draw() {
    ...
    // Halo
    ctx.strokeStyle = this.type === 'shield'
      ? 'rgba(0, 255, 120, 0.25)'
      : this.type === 'triple'
        ? 'rgba(255, 200, 0, 0.25)'
        : 'rgba(0, 255, 255, 0.25)';
    ...
    ctx.lineCap = 'round';
    if (this.type === 'shield') {
      // Escudo: contorno + cruz protectora
      ctx.strokeStyle = '#0f8';
      ctx.beginPath();
...
if (this.speedBoost    > 0) this.speedBoost    -= dt;
    ...
    const DRAG   = 0.987;
    const boost  = this.speedBoost > 0 ? SPEED_BOOST_MULT : 1;
...
// Halo del power-up "Triple Shot"
    ...
    if (this.shieldTimer > 0) {
      const blink = this.shieldTimer < 1.5 && Math.floor(this.shieldTimer * 8) % 2 === 0;
      ctx.strokeStyle = blink ? 'rgba(0,255,120,0.4)' : 'rgba(0,255,120,0.85)';
      ctx.lineWidth   = 2.5;
      ctx.beginPath();
...
function loadSkin() {
  ...
  currentSkin = Number.isInteger(saved) && saved >= 0 && saved < SKINS.length
    ? saved
    : 0;
}
...
function destroyAsteroid(a) {
  ...
  if (Math.random() < POWERUP_DROP_CHANCE && powerUps.length < POWERUP_CAP) {
    const r = Math.random();
    const type = r < 0.33 ? 'speed' : r < 0.66 ? 'shield' : 'triple';
